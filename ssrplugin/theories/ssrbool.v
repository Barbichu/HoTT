...
Coercion is_true b := b = true. (* Prop *)
...
Definition mkPredType pT toP := PredType (existT (@isMem pT toP) _ (erefl _)).
...
Definition mem T (pT : predType T) : pT -> mem_pred T :=
  nosimpl (let: PredType _ _ (existT mem _) := pT return pT -> _ in mem).
...
Definition onPhantom T (P : T -> Type) (x : T) := Phantom _ (P x).
(* Matthieu: BUG *)
